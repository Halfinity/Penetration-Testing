...
function SCAN() {
	...
	printf "${Reset}"
	sleep 2
	sudo nmap $RANGE -sV --open -T5 -oN NmapResulets.txt 1>/dev/null 2>/dev/null
	sudo nmap $RANGE -sV --open -T5 -oX NmapResulets.xml 1>/dev/null 2>/dev/null
	...
	printf "${Reset}"
	sudo masscan -iL HOSTS.txt -sU --rate=10000 > MasscanResulets.xml  1>/dev/null 2>/dev/null
	sudo masscan -iL HOSTS.txt -sU --rate=10000 > MasscanResulets.txt  1>/dev/null 2>/dev/null
...
function SEARCHSPLOIT() {
	...
	printf "${Reset}"
	searchsploit --exclude="Privilege Escalation" --nmap NmapResulets.xml > SearchsploitResults.txt  2>/dev/null
	sleep 2
	echo
	printf "${Background_Red}"
	echo "[!] Done."
